...
> 현재 점수에는 아정당 집계 데이터가 연결되어 있습니다. 다만 인구·상권·문화·코스 등 일부 신호는 개발용 보조 데이터이며, NEXT SPOT은 예측을 보장하지 않는 설명형 휴리스틱입니다.

## 아정당 데이터 적용 현황

실제 제공받은 `05_regional_new_install.csv`를 확인해 서울 25개 자치구의 2026년 7월·직전월 인터넷 계약 건수를 지역×월로 집계했습니다. 앱에서는 그중 10개 관찰 동네를 자치구에 매핑해 **신규 인터넷 설치 신호**로 사용합니다.

- 적용 원본: 아정당 `05_regional_new_install.csv`
- 적용 기간: 2023-07~2026-07 (화면 점수 비교는 최신 2개월)
- 적용 단위: 서울 자치구 × 월
- 사용 변수: `contract_count`의 최신월·직전월 합계 및 증감률
- 해석 범위: 인터넷 신규 설치는 이사·신규 가구 유입의 가능성을 보여주는 보조 신호이며 실제 이사를 의미하지 않습니다.

`src/data/providers/realDataProvider.js`가 아정당 신호를 주입하는 진입점이고, `src/data/ajeongRegionalSignals.js`는 원본 CSV를 저장하지 않고 집계한 비식별 스냅샷입니다. 공개 인구·상권 데이터가 연결되기 전까지 나머지 feature와 경험 코스는 개발용 보조 데이터로 표시합니다.

## 발표자료

[NEXT SPOT 발표자료 다운로드](outputs/넥스트스팟_발표자료_최종최수종.pptx)
...
<p align="center">
  ...
  <a href="outputs/넥스트스팟_발표자료_최종최수종.pptx">
...
- 📊 [Presentation deck](outputs/넥스트스팟_발표자료_최종최수종.pptx)
