...
app.innerHTML = `
  ...
  <main id="top">
    ...
    <section class="discovery-workspace" aria-labelledby="chat-title"><div class="workspace-toolbar"><div class="workspace-brand"><span>✦</span><b>SAVED</b></div><button type="button">↶ Undo</button><button type="button" disabled>↷ Redo</button><span class="toolbar-spacer"></span><button class="toolbar-share" type="button">↗ Share</button><button type="button">▦</button><button type="button">☷</button><button type="button">•••</button></div>
    ...
    <section class="map-section" aria-labelledby="map-title">
      ...
      <div class="map-shell"><div class="map-copy"><span class="map-copy-label">NEXT SPOT INDEX</span><strong>변화가 빠르지만<br />아직 과밀하지 않은 곳</strong><p>마커에 후보 점수와 변화 정도를 표시합니다.</p></div><div class="seoul-map" aria-label="서울의 NEXT SPOT 후보 지역 지도형 UI" id="seoul-map"></div><p class="map-disclaimer">서울 지도형 UI · 아정당 신규 설치 집계 + 개발용 보조 신호</p></div>
    </section>
...
const openDetail = (id) => {
  ...
  detailContent.innerHTML = `
    ...
    <div class="detail-top"><div><p class="eyebrow">NEIGHBORHOOD SIGNAL DETAIL</p><h2 id="detail-title">${neighborhood.name}</h2><p>${neighborhood.district} · 기준월 ${neighborhood.referenceMonth} · <b>${neighborhood.ajeongData ? '아정당 데이터 기반' : '개발용 mock 분석'}</b></p></div><div class="detail-score"><span>NEXT SPOT SCORE</span><strong>${neighborhood.score}</strong><small>설명형 휴리스틱</small></div></div>
